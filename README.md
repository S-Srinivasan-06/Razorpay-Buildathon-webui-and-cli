...
# Launch Web Console Server:
python -m src.app.server.main --host 127.0.0.1 --port 8000
# OR launch via unified CLI:
python src/cli.py --server --host 127.0.0.1 --port 8000
```
...
Demo suites are included in `datasets/` and can be loaded with **one click** from the Web Console ("Load Sample Data" button):
...
|------|------|-------------|
| `datasets/payments.csv` | 100 | Dispatch orders — Flipkart & Zomato channels |
| `datasets/bank.csv` | 80 | Bank credits — HDFC Bank & ICICI Bank |

**30 injected discrepancies:** 10 duplicate orders · 10 value/price errors · 10 missing bank credits

```bash
# CLI execution
python src/cli.py datasets/payments.csv datasets/bank.csv
```
...
```bash
# CLI execution (5-way reconciliation)
python src/cli.py \
  datasets/supply_chain_ecosystem/razorpay_x_ledger.csv \
  datasets/supply_chain_ecosystem/hdfc_corporate_statement.csv \
  datasets/supply_chain_ecosystem/icici_current_statement.csv \
  datasets/supply_chain_ecosystem/urban_nest_po.csv \
  datasets/supply_chain_ecosystem/megadist_invoice.csv
```
...
├── README.md                           # GitHub project blueprint & docs
├── datasets/                           # Bundled demo datasets outside src/
│   ├── payments.csv                    # 2-file demo payments
│   ├── bank.csv                        # 2-file demo bank statement
│   ├── ground_truth.jsonl              # Evaluation ground truth
│   └── supply_chain_ecosystem/         # 5-file enterprise ecosystem
├── data/                               # Isolated runtime storage outside src/
│   ├── audit/                          # SHA-256 tamper-evident ledgers
│   ├── logs/                           # System and session logs
│   ├── outputs/                        # Output reconciliations & journals
│   └── uploads/                        # Per-session uploaded files
├── docker-compose.yml                  # 1-click Docker launch
├── docs/                               # System technical specifications
...
├── how_to_run.txt                      # Terminal execution reference
├── pytest.ini                          # Pytest runner configuration
├── requirements.txt                    # Pinned Python dependencies
├── src/                                # Application source code
│   ├── cli.py                          # Unified terminal CLI runner
│   └── app/
│       ├── config.py                   # Environment & path management
│       ├── core/                       # State machine, SHA-256 audit, PII, LLM
│       ├── data/                       # Ecosystem & benchmark generators
│       ├── engine/                     # Matching engine, fee/tax rules, multiway
│       ├── server/                     # FastAPI server, REST API v2, WebSocket
